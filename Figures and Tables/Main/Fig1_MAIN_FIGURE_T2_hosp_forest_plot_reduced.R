...
#Forest plot of clinical and sociodemographic risk factor associations for Covid-19, influenza, and pneumonia hospitalisation in type 2 diabetes
...
row_names_with_groups <- data.frame(row_name = c("Sex", "Female", "Male", "Age group, years", "<40", "40-49","50-59", "60-69", "70-79", "80-89", "90+", "Ethnicity", "White", "South Asian", "Black", "Other", "Mixed", "Index of multiple deprivation quintile", "1 (least deprived)", "2", "3", "4", "5 (most deprived)",
                                                 "Duration of diabetes, years", "<1", "1-2", "3-5", "6-9", "10-14", "15-19", "20+", "HbA1c, mmol/mol", "<48", "48-53", "53-64", "64-75", "75-86", "86+", "Number of microvascular complications", "0 complications", "1 complication", "2 complications", "3 complications",
...
fp <- coefs %>% forestplot(labeltext = row_name,
                     ...
                     txt_gp = fpTxtGp(label = gpar(cex = 0.75), ticks  = gpar(cex = 0.75), xlab = gpar(cex = 0.75), legend = gpar(cex = 0.75), summary = gpar(cex = 0.75)),
                     xlab = "Hazard ratio",
                     is.summary = c(TRUE, rep(FALSE, 2), TRUE, rep(FALSE,7), TRUE, rep(FALSE,5), TRUE, rep(FALSE,5), TRUE, rep(FALSE,7), TRUE, rep(FALSE,6), TRUE, rep(FALSE,4), TRUE, rep(FALSE,6)),
                     legend = c("Covid", "Influenza", "Pneumonia"),
                     legend_args = fpLegend(pos = list(x=.72, y=1, align = "horizontal"))
)
fp
...
pdf("Fig1_MAIN_FIGURE_T2_hosp_forest_plot_reduced.pdf",width=10,height=10)
fp
dev.off()
