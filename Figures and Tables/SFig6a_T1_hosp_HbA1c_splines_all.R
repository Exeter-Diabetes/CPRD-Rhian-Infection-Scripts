...
model <- cph(Surv(survival_time,outcome) ~ rcs(hba1c_value,5) + gender + age_cat + eth5 + imd_quintile + region + duration_cat + bmi_cat, data = cohort, x= T, y=T)
...
options(datadist="dd")
dd$limits["Adjust to","hba1c_value"] <- 66
model <- cph(Surv(survival_time,outcome) ~ rcs(hba1c_value,3) + gender + age_cat + eth5 + imd_quintile + region + duration_cat + bmi_cat, data = cohort, x= T, y=T)
predict <- Predict(model, hba1c_value = seq(30,120, by =1), ref.zero=TRUE, fun = exp)
predict_df <- as.data.frame(predict)
