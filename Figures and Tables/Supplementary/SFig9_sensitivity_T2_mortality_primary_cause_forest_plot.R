################################################################################
#Script to produce Supplementary Figure 9
#Sensitivity analysis restricting outcome to infection as the primary cause of death
#Forest plot of all risk factor associations for Covid-19 and pneumonia mortality (primary cause) in type 2 diabetes
...
coefs2 <- model_df_pneumo %>% select(row_name, mean = exp.coef., lower = lower..95, upper = upper..95, p_value = Pr...z..)
...
tick <- c(0.1, 0.25, 0.5, 1, 2, 5, 10, 20)
...
fp <- coefs %>% forestplot(labeltext = row_name,
                           ...
                           col=fpColors(box=c("#fc8d62","#8da0cb"), lines=c("#fc8d62","#8da0cb"), zero = "gray50"), # this changes the box/ line colour- use when have multiple infections plotted
                           xticks = tick,
                           boxsize = .2,
                           ...
                           clip = c(0.05,22),
                           new_page = TRUE,
                           fn.ci_norm = c(fpDrawNormalCI, fpDrawDiamondCI), #can use this to change shape of box
                           ...
                           legend = c("Covid", "Pneumonia"),
                           legend_args = fpLegend(pos = list(x=.84, y=1, align = "horizontal"))
)

#Save as pdf
pdf.options(reset = TRUE, onefile = TRUE)
pdf("SFig9_sensitivity_T2_mortality_primary_cause_forest_plot.pdf",width=10,height=14)
fp
dev.off()
