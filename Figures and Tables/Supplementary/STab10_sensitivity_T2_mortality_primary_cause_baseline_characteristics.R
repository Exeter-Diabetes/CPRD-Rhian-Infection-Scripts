################################################################################
#Script to produce Supplementary Table 10
#Sensitivity analysis restricting outcome to infection as the primary cause of death
#Table of full baseline characteristics of 2020 and 2016 cohorts and people who died with Covid-19 and pneumonia (primary cause) with type 2 diabetes
################################################################################
...
#Combine
tabprint_covid <- tabprint1 %>% rename(All = Overall) %>% left_join(tabprint2, by = "measure") %>% select(measure, All, Deaths = Overall) %>% filter(All != "") %>% mutate(Deaths = ifelse(is.na(Deaths), "0 ( 0.0)", Deaths)) %>% rename ("2020 cohort" = All, "Covid-19 deaths" = Deaths)
...
#Combine
tabprint_pneumo <- tabprint1 %>% rename(All = Overall) %>% left_join(tabprint2, by = "measure") %>% select(measure, All, Deaths = Overall) %>% filter(All != "") %>% mutate(Deaths = ifelse(is.na(Deaths), "0 ( 0.0)", Deaths)) %>% rename ("2016 cohort" = All, "Pneumonia deaths" = Deaths)

################################################################################
##Add together 
tabprint <- tabprint_covid %>% left_join(tabprint_pneumo)
