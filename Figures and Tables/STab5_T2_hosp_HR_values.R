...
##Run models####################################################################
#Covid
model_covid <- coxph(Surv(survival_time, outcome) ~ gender + age_cat + eth5 + imd_quintile + region + duration_cat + hba1c_cat + number_complications + bmi_cat + smoking_status +
                       ...
                       treatment_6m + oralsteroids_6m + immunosuppressants_6m + labas_6m + ltras_6m, data = cohort_covid)
...
#Flu
model_flu <- coxph(Surv(survival_time, outcome) ~ gender + age_cat + eth5 + imd_quintile + region + duration_cat + hba1c_cat + number_complications + bmi_cat + smoking_status +
                     preindex_hypertension + preindex_af + preindex_angina + preindex_myocardialinfarction + preindex_revasc + preindex_ihd + 
                     preindex_heartfailure + preindex_pad + recent_hosp_resp_infect + recent_hosp_anything + preindex_asthma + preindex_copd + preindex_bronchiectasis +
                     preindex_pulmonaryfibrosis + preindex_pulmonaryhypertension + preindex_tia + preindex_stroke + preindex_dementia + preindex_otherneuroconditions +
                     preindex_haem_cancer + preindex_solid_cancer + preindex_solidorgantransplant + preindex_cld + ckd_stage + acr_cat +  
                     treatment_6m + oralsteroids_6m + immunosuppressants_6m + labas_6m + ltras_6m, data = cohort_flu)
...
#Pneumonia
model_pneumo <- coxph(Surv(survival_time, outcome) ~ gender + age_cat + eth5 + imd_quintile + region + duration_cat + hba1c_cat + number_complications + bmi_cat + smoking_status +
                        preindex_hypertension + preindex_af + preindex_angina + preindex_myocardialinfarction + preindex_revasc + preindex_ihd + 
                        preindex_heartfailure + preindex_pad + recent_hosp_resp_infect + recent_hosp_anything + preindex_asthma + preindex_copd + preindex_bronchiectasis +
                        preindex_pulmonaryfibrosis + preindex_pulmonaryhypertension + preindex_tia + preindex_stroke + preindex_dementia + preindex_otherneuroconditions +
                        preindex_haem_cancer + preindex_solid_cancer + preindex_solidorgantransplant + preindex_cld + ckd_stage + acr_cat +  
                        treatment_6m + oralsteroids_6m + immunosuppressants_6m + labas_6m + ltras_6m, data = cohort_pneumo)
...
tab <- coefs %>% filter(!is.na(mean.x)) %>%
  ...
  mutate(hr_ci_pneumo = ifelse(is.na(p_value), "1 (ref)", paste0(round(mean,digits =2)," (",round(lower,digits=2),"-",round(upper,digits=2),")")), p_value_pneumo = ifelse(p_value <0.001, "<0.001", round(p_value, digits = 3))) %>%
  select(row_name, hr_ci_covid, p_value_covid, hr_ci_flu, p_value_flu, hr_ci_pneumo, p_value_pneumo)

tab <- tab %>% rename(" " = row_name, "HR (95% CI) Covid-19" = hr_ci_covid, "p value Covid-19" = p_value_covid, "HR (95% CI) Influenza" = hr_ci_flu, "p value Influenza" = p_value_flu, "HR (95% CI) Pneumonia" = hr_ci_pneumo, "p value Pneumonia" = p_value_pneumo)
...
kableExtra::kable(tab,format="html", align="lll") %>% 
  ...
  pack_rows("Sex", 1,2, bold = TRUE) %>%
  pack_rows("Age group, years",3,9, bold=TRUE) %>% 
  pack_rows("Ethnicity",10,15, bold=TRUE)  %>% 
  pack_rows("Index of multiple deprivation quintile",16,21, bold=TRUE)  %>%  
  pack_rows("Duration of diagnosed diabetes, years",22,28, bold=TRUE)  %>%  
  pack_rows("HbA1c, mmol/mol",29,35, bold=TRUE)  %>% 
  pack_rows("Number of microvascular complications",36,39, bold=TRUE)  %>% 
  pack_rows("BMI, kg/m2",40,46, bold=TRUE)  %>%
  pack_rows("Smoking status",47,50, bold=TRUE)  %>% 
  pack_rows("Comorbidities",51,70, bold=TRUE)  %>% 
  pack_rows("Cardiovascular",51,58, bold=TRUE)  %>% 
  pack_rows("Recent hospitalisation",59,60, bold=TRUE)  %>% 
  pack_rows("Respiratory",61,62, bold=TRUE)  %>% 
  pack_rows("Neurological",63,66, bold=TRUE)  %>% 
  pack_rows("Oncological",67,68, bold=TRUE)  %>% 
  pack_rows("Other",69,70, bold=TRUE)  %>% 
  pack_rows("Chronic Kidney Disease (CKD) stage",71,77, bold=TRUE)  %>% 
  pack_rows("Albumin creatinine ratio (ACR) category",78,81, bold=TRUE)  %>% 
  pack_rows("Diabetes treatment (last 6 months)",82,84, bold=TRUE)  %>%
  ...
  cat(.,file="STab5_T2_hosp_HR_values.html")
